...
def get_hosts(current_ip, minimum, maximum):
	for end in range(minimum, maximum):
		...
		if answer.sprintf('%DNS.rcode%') == "ok":
			while True:
				try:
...
def start():
	...
	for ip in sum_dict:
		...
		if threads > 1:
			...
			for thread_counter in range(1, threads):
				exec("thread%d = myThread(\"get ports\", (thread_counter-1)*num_ports_for_thread, thread_counter*num_ports_for_thread, ip)" % (thread_counter))
				exec("thread%d.start()" % (thread_counter))
				exec("list_of_threads.append(thread%d)" % (thread_counter))
			...
			if num_of_ports_for_last_thread > 0:
				thread_counter += 1
				# last thread handling
				exec("thread%d = myThread(\"get ports\", number_of_subs - num_of_ports_for_last_thread, number_of_subs, ip)" % (thread_counter))
				exec("thread%d.start()" % (thread_counter))
				exec("list_of_threads.append(thread%d)" % (thread_counter))
		else:
